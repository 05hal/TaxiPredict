...
interface MonthData {
  ...
  geohashPoints: Array<{ geohash: string; pickups: number; latitude: number; longitude: number }>;
  mapIntensity: number[][];
          ...
          >
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className={`flex h-8 w-8 items-center justify-center rounded-2xl text-xs font-black ${isActive ? "bg-cyan-200 text-slate-950" : "bg-cyan-300/15 text-cyan-100"}`}>
                  {index + 1}
                </span>
                <div>
              ...
              <span className="text-sm font-black text-cyan-100">{item.score}%</span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-amber-200" style={{ width: `${Math.max(item.score, 5)}%` }} />
            </div>
          </Container>
        );
      })}
    ...
    <GlassCard>
      ...
      <div className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-4">
        <svg viewBox="0 0 100 100" className="h-64 w-full overflow-visible">
          ...
          {secondaryPoints ? <polyline points={secondaryPoints} fill="none" stroke="#facc15" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" /> : null}
          <polyline points={primaryPoints} fill="none" stroke="#22d3ee" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round" />
          {primary.map((value, index) => {
            const [x, y] = toPoint(value, index).split(",");
            return <circle key={`${labels[index]}-${value}`} cx={x} cy={y} r="1.8" fill="#e0f2fe" stroke="#0891b2" strokeWidth="0.8" />;
          })}
        </svg>
...
function TreeModelRegionChart({ region }: { region: TreeModelRegion }) {
  ...
  const span = Math.max(maxValue - minValue, 1);
  const labelIndexes = [0, Math.floor((region.points.length - 1) / 2), region.points.length - 1];

  function toPoint(value: number, index: number) {
    const x = region.points.length <= 1 ? 50 : (index / (region.points.length - 1)) * 100;
    const y = 92 - ((value - minValue) / span) * 78;
    return `${x},${y}`;
  }

  const actualPoints = region.points.map((item, index) => toPoint(item.actual, index)).join(" ");
  const predictedPoints = region.points.map((item, index) => toPoint(item.predicted, index)).join(" ");
  ...
  return (
    <div className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-4">
      <svg viewBox="0 0 100 100" className="h-72 w-full overflow-visible">
        ...
        <polyline points={predictedPoints} fill="none" stroke="#facc15" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
        <polyline points={actualPoints} fill="none" stroke="#22d3ee" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
      <div className="mt-2 grid grid-cols-3 text-[10px] text-slate-500">
        {labelIndexes.map((index, labelIndex) => (
          <span key={`${region.region}-${region.points[index]?.time}`} className={labelIndex === 1 ? "text-center" : labelIndex === 2 ? "text-right" : ""}>
        ...
        {data.map((item) => {
          ...
          return (
            <div
              ...
              style={{ backgroundColor: `rgba(34, 211, 238, ${0.12 + intensity * 0.72})` }}
            >
...
function HeatGrid({ data }: { data: HeatmapPoint[] }) {
  const xLabels = Array.from({ length: 24 }, (_, index) => String(index).padStart(2, "0"));
  const yLabels = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"];
  const valueMap = new Map(data.map((item) => [`${item.y}|${item.x}`, item.value]));
  const maxValue = Math.max(...data.map((item) => item.value), 1);
  ...
  return (
    <GlassCard>
      ...
      <div className="space-y-2">
        {yLabels.map((weekday) => (
          <div key={weekday} className="grid grid-cols-[2.5rem_1fr] items-center gap-3">
    ...
    <div className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-4">
      <svg viewBox="0 0 100 100" className="h-64 w-full">
        {[20, 40, 60, 80].map((value) => (
          <g key={value}>
            <line x1="8" x2="96" y1={value} y2={value} stroke="rgba(255,255,255,0.08)" strokeWidth="0.5" />
            <line x1={value} x2={value} y1="8" y2="92" stroke="rgba(255,255,255,0.08)" strokeWidth="0.5" />
          </g>
        ))}
        {data.map((item, index) => {
          const x = 8 + ((item.x - minX) / xSpan) * 88;
          const y = 92 - ((item.y - minY) / ySpan) * 84;
          return <circle key={`${item.x}-${item.y}-${index}`} cx={x} cy={y} r={item.size ?? 4} fill="rgba(34,211,238,0.72)" stroke="#fef3c7" strokeWidth="0.8" />;
        })}
      </svg>
          ...
          row.map((value, colIndex) => {
            ...
            return (
              <div
                ...
                style={{ backgroundColor: `rgba(34, 211, 238, ${0.08 + intensity * 0.78})` }}
              >
                <span className="text-[10px] font-bold text-white/80">{formatNumber(value)}</span>
              </div>
            );
          }),
        )}
              ...
              <div className="mt-5 grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
                <GlassCard>
                  <div className="mb-5">
                    ...
                    <h3 className="mt-1 text-2xl font-black text-white">星期需求画像</h3>
                  </div>
                ...
                <GlassCard>
                  <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <h3 className="mt-1 text-2xl font-black text-white">Geohash 热点分布</h3>
                    </div>
                    <Tag color="cyan" className="rounded-full">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {dashboardData.geohashPoints.slice(0, 10).map((item, index) => (
                      <div key={item.geohash} className="rounded-3xl border border-white/10 bg-slate-950/60 p-4">
                        <div className="mb-2 flex items-center justify-between">
                          <span className="font-mono text-sm font-black text-white">{item.geohash}</span>
                          <span className="rounded-full bg-cyan-300/15 px-3 py-1 text-xs font-bold text-cyan-100">#{index + 1}</span>
                        </div>
                        <p className="text-lg font-black text-white">{formatNumber(item.pickups)} 单</p>
              ...
              <div className="grid gap-5">
                <div className="grid gap-5 lg:grid-cols-[1fr_0.9fr]">
                  <GlassCard>
                    <div className="mb-5">
                      ...
                      <h3 className="mt-1 text-2xl font-black text-white">天气散点关系</h3>
                    </div>
                    <ScatterPlot data={dashboardData.weatherScatter} xLabel="温度（°C）" yLabel="订单量" />
                  </GlassCard>
                  <GlassCard>
                    <div className="mb-5">
                      <p className="text-sm text-slate-300">天气类别</p>
                      <h3 className="mt-1 text-2xl font-black text-white">平均订单影响</h3>
                    </div>
                ...
                <GlassCard>
                  <div className="mb-5">
                    ...
                    <h3 className="mt-1 text-2xl font-black text-white">不同温度区间平均需求</h3>
                  </div>
              ...
              <div className="mb-5 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
                ...
                <GlassCard>
                  <div className="mb-5">
                    ...
                    <h3 className="mt-1 text-2xl font-black text-white">拟合散点</h3>
                  </div>
                  <ScatterPlot data={dashboardData.actualPredictedScatter} xLabel="真实订单量" yLabel="预测订单量" />
                </GlassCard>
              </div>
              <div className="mb-5 grid gap-5 lg:grid-cols-3">
                <GlassCard>
                  <div className="mb-5">
                    ...
                    <h3 className="mt-1 text-2xl font-black text-white">误差分布</h3>
                  </div>
                  <SimpleBarChart data={dashboardData.errorHistogram} valueSuffix=" 个" />
                </GlassCard>
                <LineChartCard
                  title="残差走势"
                  subtitle="预测值减真实值"
                  labels={dashboardData.residualTrend.map((item) => item.label)}
                  primary={dashboardData.residualTrend.map((item) => item.value)}
                />
                <GlassCard>
                  <div className="mb-5">
                    <p className="text-sm text-slate-300">误差日期</p>
                    <h3 className="mt-1 text-2xl font-black text-white">偏差排行</h3>
                  </div>
